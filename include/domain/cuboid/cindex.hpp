...
template<St DIM>
class CIndex_: public std::array<int, DIM> {
public:
    static const St Dim = DIM;
    typedef CIndex_<DIM> Self;
public:
    CIndex_() {
        this->fill(0);
    }
    CIndex_(int a, int b = 0, int c = 0) {
        this->at(0) = a;
        if (Dim >= 2) {
            this->at(1) = b;
        }
    ...
    CIndex_(const CIndex_<DIM>& other) {
        for (St d = 0; d < Dim; ++d) {
            this->at(d) = other[d];
        }
    }
    ...
    bool operator==(const CIndex_<DIM>& other) const {
        bool res = true;
        for (St d = 0; d < Dim; ++d) {
...
template<St DIM>
struct CIndex_compare_ {
    typedef CIndex_<DIM> Index;
...
template<St DIM>
std::ostream& operator<<(std::ostream& stream, const CIndex_<DIM>& index) {
    stream << "(";
    for (St d = 0; d < DIM; ++d) {
        stream << index[d];
        if (d != DIM - 1) {
...
template<St DIM>
void Shift(CIndex_<DIM>& index, St dim, St ori, St step = 1) {
    if(ori == _M_){
