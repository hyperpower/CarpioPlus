...
public:
    ...
    typedef Node_<DATA, DIM> Self;
    typedef Node_<DATA, DIM> *pSelf;
    typedef Cell_<Vt, Dim> Cell;
    typedef Cell_<Vt, Dim> *pCell;
    typedef DATA  Data;
    typedef Data *pData;
    ...
    typedef void (*pFun_Conditional)(ArrayListV_<int> &, pNode, utPointer);
...
protected:
    int _height(const pNode Current) const {
        ...
        if (!Current->has_child()) {
            ...
            ArrayListV_<St> arrh(NumChildren);
...
public:
     ...
     *  constructor
     */
    Node_(pNode f, int nt, St level, St root_idx, St c_idx,   //
    ...
    Node_(pNode f, int nt, St level, St root_idx, St child_idx,  //
            const Cell& c) {
        _node_type = nt;
        _level = level;
        cell = new Cell(c);
...
public:
    ...
    Vt cp(Axes axes) const {  //center point
        if (Dim <= 2 && axes == _Z_) {
            return 0.0;
        }
        return this->cell->get(_C_, axes);
    }
    Vt d(Axes axes) const {  //d
        return this->cell->get_d(axes);
    }
    Vt hd(Axes axes) const {  //d
        return this->cell->get_hd(axes);
    }
    Vt p(Orientation ori, Axes axes) const {  //point
        if (Dim <= 2 && axes == _Z_) {
            return 0.0;
        }
        return this->cell->get(ori, axes);
    }
    Vt p(Direction dir, Axes axes) const {
        if (Dim <= 2 && axes == _Z_) {
            return 0.0;
        }
        return this->cell->get(ToOrientation(dir, axes), axes);
    }
    Vt face_area(Direction dir) const {
    ...
    static std::list<pNode> GetLeaf(pNode pn, Axes a, Vt cor) {
        ASSERT(pn != nullptr);
        if (!pn->cell->is_in_on(a, cor, _co_)) {
            return std::list<pNode>();
        }
        std::function<void(bool[], pNode, Vt)> condition =
                [a](bool arr[], pNode pn, Vt cor ) {
                    for(St i = 0; i<NumChildren;++i) {
...
template<typename DATA, St DIM>
const Node_<DATA, DIM> *
GetpNodeSiblingPlus(const Node_<DATA, DIM> *p) {
...
template<typename DATA, St DIM> Node_<DATA, DIM> *
GetpNodeSiblingPlus(Node_<DATA, DIM> *p) {
    typedef Node_< DATA, DIM> Node;
    Node* f = p->father;
    if (f == nullptr) {
        return p;
...
template<typename DATA, St DIM> Node_<DATA, DIM> *
GetFirstChild(Node_< DATA, DIM> * p) {
    typedef Node_<DATA, DIM> Node;
    Node* c = nullptr;
    if (p == nullptr) {
        return c;
...
template<typename DATA, St DIM>
const Node_<DATA, DIM> *
GetFirstChild(const Node_<DATA, DIM> * p) {
    typedef Node_<DATA, DIM> Node;
    Node* c = nullptr;
    if (p == nullptr) {
        return c;
...
template<typename DATA, St DIM>
const Node_<DATA, DIM> *
GetFirstLeaf(const Node_<DATA, DIM> * p) {
    typedef Node_< DATA, DIM> Node;
...
FaceType GetFaceType(  //
        const Node_<DATA, DIM>* p,     //main node
        const Node_<DATA, DIM>* pn) {
    if (p == nullptr) {
        return _Null_;
    }
