...
public:
	...
	typedef Grid_<DATA, DIM> Self;
	typedef Grid_<DATA, DIM> *pSelf;
	typedef const Grid_<DATA, DIM> * const_pSelf;
	typedef Cell_<Vt, Dim> Cell;
	typedef Cell *pCell;
	typedef DATA Data;
	typedef Data *pData;
	typedef Node_<DATA, DIM> Node;
	typedef Node_<DATA, DIM>& ref_Node;
	typedef const Node_<DATA, DIM>& const_ref_Node;
	typedef const Node_<DATA, DIM> const_Node;
	typedef Node_<DATA, DIM> *pNode;
	typedef const Node_<DATA, DIM> * const_pNode;

//	typedef Path_<Dim> Path;
	typedef Face_<Node, pNode> Face;
	typedef Face_<Node, pNode> *pFace;
	typedef typename MultiArray_<pNode, Dim>::reference reference;
	typedef typename MultiArray_<pNode, Dim>::const_reference const_reference;
	typedef typename MultiArray_<pNode, Dim>::size_type size_type;

	typedef void (*pfunction)(pNode, utPointer);

	typedef void (*pfunction_conditional)(arrayList &, pNode, utPointer);

	typedef typename MultiArray_<pNode, Dim>::iterator iterator;
	typedef typename MultiArray_<pNode, Dim>::const_iterator const_iterator;

	/*
	 *  data
	 */
	MultiArray_<pNode, Dim> nodes;

	/*
	 *  constructor
	 */

	Grid_(  St ni,     Vt ox,     Vt dx,       //
			St nj = 0, Vt oy = 0, Vt dy = 0,   //
			St nk = 0, Vt oz = 0, Vt dz = 0) :
			nodes(ni, nj, nk) {
		St i_1d = 0;
		for (St i = 0; i < ni; i++) {
...
//	}
	 ...
	 */
protected:
	template<typename DATA, St D, class _Ref, class _Ptr>
	class iterator_leaf_ {
	public:
		typedef DATA Data;
		typedef Node_<DATA, D> Node;
		typedef Node_<DATA, D> *pNode;
		typedef const Node_<DATA, D> const_Node;
		typedef const Node_<DATA, D>* const_pNode;

		typedef iterator_leaf_<DATA, D, Node&, pNode> iterator;
		typedef iterator_leaf_<DATA, D, const_Node&, const_pNode> const_iterator;
		typedef iterator_leaf_<DATA, D, _Ref, _Ptr> Self;

		typedef Grid_<DATA, D> Grid;
		typedef Grid_<DATA, D>* pGrid;
		typedef const Grid_<DATA, D>* const_pGrid;
	...
	public:
		...
		//typedef Face_<DATA, D> Face;
		//typedef Face_<DATA, D> *pFace;
		//typedef const Face_<DATA, D> const_Face;
		//typedef const Face_<DATA, D>* const_pFace;

		typedef iterator_face_<DATA, D, Node&, pNode> iterator;
		typedef iterator_face_<DATA, D, const_Node&, const_pNode> const_iterator;
		typedef iterator_face_<DATA, D, _Ref, _Ptr> Self;
...
public:
	 ...
	 */
	typedef iterator_leaf_<Data, Dim, Node&, Node*> iterator_leaf;
	typedef iterator_leaf_<Data, Dim, const Node&, const Node*> const_iterator_leaf;

	typedef iterator_face_<Data, Dim, Node&, Node*> iterator_face;
	typedef iterator_face_<Data, Dim, const Node&, Node*> const_iterator_face;
	...
	pNode get_pnode(const Vt& x, const Vt& y, const Vt& z = 0.0) {
	 ...
	 * get leaf on Axes
	 */
	std::list<pNode> get_leaf(Axes aix, const Vt& x) {
		std::list<pNode> ret;
		for (const_iterator iter = this->begin(); iter != this->end(); ++iter) {
			pNode pn = (*iter);
			std::list<pNode> lp;
