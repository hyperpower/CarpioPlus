...
#include "domain/structure/grid/sgrid_center_data.hpp"
#include "regular.hpp"
...
template<St DIM>
class SGhostCut_: public SGhostRegular_<DIM> {
public:
...
//	typedef std::shared_ptr<Cell> spCell;
	...
	typedef SGridCenterData_<int, DIM> CenterData;
	typedef SGridEdgeData_<Vt, DIM>   EdgeData;
...
protected:
	CenterData _center_data;
	EdgeData   _edge_data;
public:
	SGhostCut_(spGrid spg): Base(spg),
    	_center_data(spg), _edge_data(spg){
	}
	~SGhostCut_() {
	}
