...
TEST(poisson, DISABLED_flower_mask) {
	const St DIM = 2;
...
TEST(poisson, flower_cut) {
	...
	St bid = 10;
	auto spgm = cg.ghost_linear_cut(spsg, fun, 0.0, 0.0, bid);
	typename Domain::spOrder sporder(new SOrderXYZ_<2>(spsg, spgm));
	...
	gnu.add(GA::WireFrameCutGhostSide(*spgm, 3));
	gnu.add(GA::WireFrameCutNormalSide(*spgm, 5));
	gnu.add(GA::WireFrameCutInterface(*spgm, 4));
	gnu.plot();
